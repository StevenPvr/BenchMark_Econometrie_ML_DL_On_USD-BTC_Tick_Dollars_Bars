...
@patch("src.clear_features.main.NonLinearCorrelationAnalyzer")
@patch("src.clear_features.main.WeightedPCAReducer")
@patch("src.clear_features.main.LogTransformer")
@patch("src.clear_features.main.ScalerApplier")
def test_run_full_pipeline(
    mock_scaler, mock_log, mock_pca, mock_corr,
    ...
    # Mock analyzer
    mock_corr_instance = mock_corr.return_value
    mock_corr_result = mock_corr_instance.analyze.return_value
    mock_corr_result.clusters = []
    mock_corr_result.method = "spearman"
    mock_corr_result.threshold_used = 0.8
    mock_corr_result.unclustered_features = []
    ...
    mock_log_result = mock_log_instance.fit.return_value
    mock_log_result.features_transformed = []
    mock_log_result.features_skipped = []
    mock_log_instance.transform.return_value = mock_df
    ...
    # Verify method calls
    assert mock_corr_instance.analyze.called
    assert mock_pca_instance.fit.called
    assert mock_log_instance.fit.called

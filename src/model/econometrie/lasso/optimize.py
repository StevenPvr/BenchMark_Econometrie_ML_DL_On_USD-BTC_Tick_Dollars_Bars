"""Hyperparameter optimization for Lasso Classifier on the train split only."""
...
def parse_args() -> argparse.Namespace:
    """Parse CLI arguments for Lasso hyperparameter search."""
    parser = argparse.ArgumentParser(description="Optimize Lasso hyperparameters on train split")
    parser.add_argument(
        "--n-trials",
        type=int,
        default=100,
...
def main() -> None:
    ...
    logger.info("=" * 60)
    logger.info("LASSO CLASSIFIER HYPERPARAMETER OPTIMIZATION (train split)")
    logger.info("=" * 60)
    ...
    save_json_pretty(opt_result.best_params, LASSO_CLASSIFIER_BEST_PARAMS_FILE)
    summary_path = LASSO_CLASSIFIER_ARTIFACTS_DIR / "optimization_results.json"
    save_optimization_results(opt_result, summary_path)

    logger.info("Optimization complete. Best params saved to %s", LASSO_CLASSIFIER_BEST_PARAMS_FILE)

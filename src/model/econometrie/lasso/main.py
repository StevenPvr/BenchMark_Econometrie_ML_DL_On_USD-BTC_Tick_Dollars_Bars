"""Main entry point for Lasso pipeline.

Usage:
    python -m src.model.econometrie.lasso.main [--n-trials N] [--skip-optim]

Example:
    # Run full optimization (50 trials by default)
    python -m src.model.econometrie.lasso.main

    # Quick run with 10 trials
    python -m src.model.econometrie.lasso.main --n-trials 10

    # Skip optimization, use default params
    python -m src.model.econometrie.lasso.main --skip-optim
"""
...
from src.path import DATASET_FEATURES_LINEAR_PARQUET, LASSO_DIR
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="Run Lasso optimization pipeline")
    parser.add_argument(
        "--n-trials",
        type=int,
        default=50,
    ...
    logger.info("=" * 60)
    logger.info("LASSO OPTIMIZATION PIPELINE")
    logger.info("=" * 60)
    ...
    logger.info("Selected features: %d/%d", result.n_selected_features, len(X_train.columns))

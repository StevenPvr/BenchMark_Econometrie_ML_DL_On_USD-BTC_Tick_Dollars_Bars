"""Evaluate a trained Lasso Classifier on the held-out test split."""
...
from src.path import (
    ...
    LASSO_CLASSIFIER_TEST_EVAL_FILE,
)
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(description="Evaluate Lasso Classifier on test split")
    parser.add_argument(
        "--model-path",
        type=Path,
        default=LASSO_CLASSIFIER_MODEL_FILE,
        help=f"Path to trained model (default: {LASSO_CLASSIFIER_MODEL_FILE})",
    )
    return parser.parse_args()


def main() -> None:
    """Load trained Lasso Classifier and evaluate on test split only."""
    args = parse_args()
    setup_logging()

    logger.info("=" * 60)
    logger.info("LASSO CLASSIFIER EVALUATION (test split only)")
    logger.info("=" * 60)
    ...
    model = LassoClassifierModel.load(model_path)
    ...
    save_evaluation_results(eval_result, LASSO_CLASSIFIER_TEST_EVAL_FILE)

    logger.info("Evaluation complete on %d samples", eval_result.n_samples)
    for metric, value in eval_result.metrics.items():
        logger.info("  %s: %.6f", metric, value)
    logger.info("Saved evaluation results to %s", LASSO_CLASSIFIER_TEST_EVAL_FILE)

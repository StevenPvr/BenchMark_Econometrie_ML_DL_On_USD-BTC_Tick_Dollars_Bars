"""Train Ridge Classifier on the train split only and save artifacts."""
...
from src.path import (
    ...
    RIDGE_CLASSIFIER_TRAINING_RESULTS_FILE,
)
...
def parse_args() -> argparse.Namespace:
    """Parse CLI arguments for Ridge Classifier training."""
    parser = argparse.ArgumentParser(description="Train Ridge Classifier on train split only")
    parser.add_argument(
        "--n-splits",
        type=int,
        default=5,
    ...
    parser.add_argument(
        "--params",
        type=Path,
        default=RIDGE_CLASSIFIER_BEST_PARAMS_FILE,
        help=f"Path to best params JSON (default: {RIDGE_CLASSIFIER_BEST_PARAMS_FILE})",
    )
...
def main() -> None:
    """Run Ridge Classifier training on the train split."""
    args = parse_args()
    setup_logging()

    logger.info("=" * 60)
    logger.info("RIDGE CLASSIFIER TRAIN (train split only)")
    logger.info("=" * 60)
    ...
    # Save artifacts
    model.save(RIDGE_CLASSIFIER_MODEL_FILE)
    save_json_pretty(best_params, RIDGE_CLASSIFIER_BEST_PARAMS_FILE)
    save_training_results(train_result, RIDGE_CLASSIFIER_TRAINING_RESULTS_FILE)

    logger.info("Training complete. Model saved to %s", RIDGE_CLASSIFIER_MODEL_FILE)
    logger.info("Training metrics saved to %s", RIDGE_CLASSIFIER_TRAINING_RESULTS_FILE)

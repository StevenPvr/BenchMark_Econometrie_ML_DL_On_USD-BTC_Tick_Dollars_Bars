"""Main entry point for Logistic (no penalty) pipeline.

Usage:
    python -m src.model.econometrie.ols.main [--n-trials N] [--skip-optim]

Example:
    # Run full optimization (20 trials by default)
    python -m src.model.econometrie.ols.main

    # Quick run with 10 trials
    python -m src.model.econometrie.ols.main --n-trials 10

    # Skip optimization, use default params
    python -m src.model.econometrie.ols.main --skip-optim
"""
...
from src.path import DATASET_FEATURES_LINEAR_PARQUET, LOGISTIC_DIR
...
def main() -> None:
    """Run the Logistic (no penalty) pipeline."""
    parser = argparse.ArgumentParser(description="Run Logistic optimization pipeline")
    parser.add_argument(
        "--n-trials",
        type=int,
        default=20,
    ...
    logger.info("=" * 60)
    logger.info("LOGISTIC (NO PENALTY) OPTIMIZATION PIPELINE")
    logger.info("=" * 60)
    ...
    # Configure pipeline
    output_dir = LOGISTIC_DIR / "results"

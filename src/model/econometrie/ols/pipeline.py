"""Complete pipeline for Logistic Regression (no penalty): optimization, training, and evaluation.
...
Note: Logistic (no penalty) is the classification equivalent of OLS.
...
@dataclass
class LogisticPipelineConfig:
    """Configuration for the Logistic Regression (no penalty) pipeline.
    ...
    n_trials: int = 20  # Logistic has fewer params, so fewer trials needed
    n_splits: int = 5
    purge_gap: int = 5
...
@dataclass
class LogisticPipelineResult:
    """Complete result from the Logistic Regression (no penalty) pipeline.
    ...
    def summary(self) -> str:
        ...
        lines = [
            "=" * 60,
            "Logistic Regression (No Penalty) Pipeline Results",
            "=" * 60,
            "",
        ]
...
class LogisticPipeline:
    """Complete pipeline for Logistic Regression (no penalty): optimize, train, evaluate.

    Note: Logistic (no penalty) is the classification equivalent of OLS.
    The optimization only tunes preprocessing options (fit_intercept, normalize).

    Supports De Prado's triple-barrier labeling (-1, 0, 1).

    Example:
        >>> pipeline = LogisticPipeline(config=LogisticPipelineConfig(
        ...     n_trials=20,
        ...     n_splits=5,
        ...     purge_gap=10,
        ... ))
    ...
    def __init__(self, config: LogisticPipelineConfig | None = None) -> None:
        """Initialize the pipeline.

        Args:
            config: Pipeline configuration.
        """
        self.config = config or LogisticPipelineConfig()

    def optimize(
        self,
        X: np.ndarray | pd.DataFrame,
        y: np.ndarray | pd.Series,
        hyperparam_space: LogisticHyperparams | None = None,
    ) -> OptimizationResult:
        ...
        space = hyperparam_space or LogisticHyperparams()
    ...
    def train(
        ...
        params: dict[str, Any],
        use_cv: bool = True,
    ) -> tuple[LogisticModel, TrainingResult | CrossValidationTrainingResult]:
    ...
    def run(
        ...
        Returns:
            LogisticPipelineResult with all results.
        """
        if self.config.verbose:
            logger.info("=" * 60)
        ...
        if skip_optimization:
            if default_params is None:
                default_params = {"fit_intercept": True, "normalize": True}
...
def run_logistic_pipeline(
    ...
    """Convenience function to run the complete Logistic Regression pipeline.
    ...
    Example:
        >>> result = run_logistic_pipeline(X_train, y_train, X_test, y_test)
...
def quick_logistic(
    ...
    """
    model = LogisticModel(normalize=normalize)

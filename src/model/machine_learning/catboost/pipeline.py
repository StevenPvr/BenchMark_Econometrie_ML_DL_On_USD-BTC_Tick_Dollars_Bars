"""Complete pipeline for CatBoost model: optimization, training, and evaluation.
...
@dataclass
class CatBoostPipelineConfig:
    """Configuration for the CatBoost pipeline.
...
@dataclass
class CatBoostPipelineResult:
    """Complete result from the CatBoost pipeline.
...
class CatBoostPipeline:
    """Complete pipeline for CatBoost: optimize, train, evaluate.

    Example:
        >>> pipeline = CatBoostPipeline(config=CatBoostPipelineConfig(
        ...     n_trials=50,
        ...     n_splits=5,
        ...     purge_gap=10,
        ... ))
    ...
    def __init__(self, config: CatBoostPipelineConfig | None = None) -> None:
        """Initialize the pipeline.

        Args:
            config: Pipeline configuration.
        """
        self.config = config or CatBoostPipelineConfig()

    def optimize(
        self,
        X: np.ndarray | pd.DataFrame,
        y: np.ndarray | pd.Series,
        hyperparam_space: CatBoostHyperparams | None = None,
    ) -> OptimizationResult:
        ...
        space = hyperparam_space or CatBoostHyperparams()
        ...
        optimizer = OptunaOptimizer(
            ...
            fixed_model_params={"random_seed": self.config.random_state},
        )
    ...
    def train(
        ...
        params: dict[str, Any],
        use_cv: bool = True,
    ) -> tuple[CatBoostModel, TrainingResult | CrossValidationTrainingResult]:
        ...
        """
        if self.config.verbose:
            logger.info("Training CatBoost with optimized parameters...")
    ...
    def evaluate(
        ...
        """
        if self.config.verbose:
            logger.info("Evaluating CatBoost on test set...")
    ...
    def run(
        ...
        Returns:
            CatBoostPipelineResult with all results.
        """
        if self.config.verbose:
            logger.info("=" * 60)
            logger.info("Starting CatBoost Pipeline")
            logger.info("=" * 60)
...
def run_catboost_pipeline(
    ...
    """Convenience function to run the complete CatBoost pipeline.
    ...
    Example:
        >>> result = run_catboost_pipeline(X_train, y_train, X_test, y_test)
...
def quick_catboost(
    ...
    X_test: np.ndarray | pd.DataFrame,
    y_test: np.ndarray | pd.Series,
) -> tuple[CatBoostModel, dict[str, float]]:

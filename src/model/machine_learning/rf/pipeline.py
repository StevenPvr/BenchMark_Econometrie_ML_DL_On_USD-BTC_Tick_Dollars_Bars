"""Complete pipeline for Random Forest model: optimization, training, and evaluation.
...
class RandomForestPipelineConfig:
    """Configuration for the Random Forest pipeline.
...
@dataclass
class RandomForestPipelineResult:
    """Complete result from the Random Forest pipeline.
...
class RandomForestPipeline:
    """Complete pipeline for Random Forest: optimize, train, evaluate.

    Example:
        >>> pipeline = RandomForestPipeline(config=RandomForestPipelineConfig(
        ...     n_trials=50,
        ...     n_splits=5,
        ...     purge_gap=10,
        ... ))
    ...
    def __init__(self, config: RandomForestPipelineConfig | None = None) -> None:
        """Initialize the pipeline.

        Args:
            config: Pipeline configuration.
        """
        self.config = config or RandomForestPipelineConfig()

    def optimize(
        self,
        X: np.ndarray | pd.DataFrame,
        y: np.ndarray | pd.Series,
        hyperparam_space: RandomForestHyperparams | None = None,
    ) -> OptimizationResult:
        ...
        space = hyperparam_space or RandomForestHyperparams()
    ...
    def train(
        ...
        params: dict[str, Any],
        use_cv: bool = True,
    ) -> tuple[RandomForestModel, TrainingResult | CrossValidationTrainingResult]:
    ...
    def run(
        ...
        Returns:
            RandomForestPipelineResult with all results.
        """
        if self.config.verbose:
            logger.info("=" * 60)
        ...
        result = RandomForestPipelineResult(
    ...
    def _save_results(
        ...
        if opt_result is not None:
            save_optimization_results(opt_result, output_dir / "optimization_results.json")

        # Save evaluation results
        save_evaluation_results(eval_result, output_dir / "evaluation_results.json")

        # Save training results
        save_training_results(train_result, output_dir / "training_results.json")
...
def run_random_forest_pipeline(
    ...
    """Convenience function to run the complete Random Forest pipeline.
    ...
    Example:
        >>> result = run_random_forest_pipeline(X_train, y_train, X_test, y_test)
...
def quick_random_forest(
    X_train: np.ndarray | pd.DataFrame,
    y_train: np.ndarray | pd.Series,
    X_test: np.ndarray | pd.DataFrame,
    y_test: np.ndarray | pd.Series,
) -> tuple[RandomForestModel, dict[str, float]]:

"""Main entry point for Random Forest pipeline.

Usage:
    python -m src.model.machine_learning.rf.main [--n-trials N] [--skip-optim]

Example:
    # Run full optimization (50 trials by default)
    python -m src.model.machine_learning.rf.main

    # Quick run with 10 trials
    python -m src.model.machine_learning.rf.main --n-trials 10

    # Skip optimization, use default params
    python -m src.model.machine_learning.rf.main --skip-optim
"""
...
from src.path import DATASET_FEATURES_PARQUET, RANDOM_FOREST_DIR
...
def load_data() -> tuple[pd.DataFrame, pd.DataFrame, pd.Series, pd.Series]:
    ...
    """
    logger.info("Loading data from %s", DATASET_FEATURES_PARQUET)

    if not DATASET_FEATURES_PARQUET.exists():
        raise FileNotFoundError(
            f"Dataset not found: {DATASET_FEATURES_PARQUET}. "
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="Run Random Forest optimization pipeline")
    parser.add_argument(
        "--n-trials",
        type=int,
        default=50,
    ...
    logger.info("=" * 60)
    logger.info("RANDOM FOREST OPTIMIZATION PIPELINE")
    logger.info("=" * 60)
    ...
    # Configure pipeline
    output_dir = RANDOM_FOREST_DIR / "results"

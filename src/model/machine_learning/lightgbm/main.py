"""Main entry point for LightGBM pipeline.

Usage:
    python -m src.model.machine_learning.lightgbm.main [--n-trials N] [--skip-optim]

Example:
    # Run full optimization (50 trials by default)
    python -m src.model.machine_learning.lightgbm.main

    # Quick run with 10 trials
    python -m src.model.machine_learning.lightgbm.main --n-trials 10

    # Skip optimization, use default params
    python -m src.model.machine_learning.lightgbm.main --skip-optim
"""
...
from src.path import DATASET_FEATURES_PARQUET, LIGHTGBM_DIR
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="Run LightGBM optimization pipeline")
    parser.add_argument(
        "--n-trials",
        type=int,
        default=50,
    ...
    logger.info("=" * 60)
    logger.info("LIGHTGBM OPTIMIZATION PIPELINE")
    logger.info("=" * 60)
    ...
    # Configure pipeline
    output_dir = LIGHTGBM_DIR / "results"

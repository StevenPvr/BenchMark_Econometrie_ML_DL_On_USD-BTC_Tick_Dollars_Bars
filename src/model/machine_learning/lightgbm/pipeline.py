"""Complete pipeline for LightGBM model: optimization, training, and evaluation.
...
class LightGBMPipelineConfig:
    """Configuration for the LightGBM pipeline.
...
@dataclass
class LightGBMPipelineResult:
    """Complete result from the LightGBM pipeline.
...
class LightGBMPipeline:
    """Complete pipeline for LightGBM: optimize, train, evaluate.

    Example:
        >>> pipeline = LightGBMPipeline(config=LightGBMPipelineConfig(
        ...     n_trials=50,
        ...     n_splits=5,
        ...     purge_gap=10,
        ... ))
    ...
    def __init__(self, config: LightGBMPipelineConfig | None = None) -> None:
        """Initialize the pipeline.

        Args:
            config: Pipeline configuration.
        """
        self.config = config or LightGBMPipelineConfig()

    def optimize(
        self,
        X: np.ndarray | pd.DataFrame,
        y: np.ndarray | pd.Series,
        hyperparam_space: LightGBMHyperparams | None = None,
    ) -> OptimizationResult:
        ...
        space = hyperparam_space or LightGBMHyperparams()
    ...
    def train(
        ...
        params: dict[str, Any],
        use_cv: bool = True,
    ) -> tuple[LightGBMModel, TrainingResult | CrossValidationTrainingResult]:
        ...
        """
        if self.config.verbose:
            logger.info("Training LightGBM with optimized parameters...")
    ...
    def run(
        ...
        Returns:
            LightGBMPipelineResult with all results.
        """
        if self.config.verbose:
            logger.info("=" * 60)
            logger.info("Starting LightGBM Pipeline")
            logger.info("=" * 60)
...
def run_lightgbm_pipeline(
    ...
    """Convenience function to run the complete LightGBM pipeline.
